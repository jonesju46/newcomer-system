...
$("newcomerForm").addEventListener("submit", e => {
  ...
  const record = {
    ...
    receptionist: $("receptionist").value.trim(),
    note: $("note").value.trim(),
    createdAt: new Date().toISOString()
  };

  if (!record.visitDate || !record.name || !record.gender || !record.ageGroup || !record.city || !record.district || !record.churchArea) {
    alert("請完成必填欄位：日期、姓名、性別、年齡區間、居住地、所屬區域");
    return;
  }
  ...
  e.target.reset();
  $("visitDate").value = today();
  $("district").innerHTML = `<option value="">請先選擇縣市</option>`;
  alert("已新增留名紀錄");
});
